...
// ═══════════════════════════════════════════════════════════
// DB CONTRACT TYPES (DB karar verir, frontend gösterir)
// ═══════════════════════════════════════════════════════════
interface Player {
  user_id: string;
  rank: number;
...
function PodiumCard({ player, place }: { player: Player; place: 1 | 2 | 3 }) {
  ...
  return (
    <div className={`animate-slide-up podium-${place === 1 ? "1st" : place === 2 ? "2nd" : "3rd"}`} style={{ order: c.order, animationDelay: c.animDelay }}>
      <div className={c.glowClass || undefined} style={{ padding: c.card.pad, borderRadius: c.card.radius, border: c.card.border, background: c.card.bg, backdropFilter: `blur(${c.card.blur})`, textAlign: "center", boxShadow: c.card.shadow, transition: "transform 0.3s", cursor: "default" }}
        ...
        <h2 style={{ fontSize: c.name.size, fontWeight: c.name.weight, marginBottom: c.name.mb, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{player.full_name}</h2>
...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    ...
    if (metaDesc) {
      metaDesc.setAttribute("content", `Compete for ${activeTab === 'weekly' ? '15 free credits' : '£1,000 prize'}. View ${activeTab} VibraXX leaderboard rankings.`);
    }
  }, [activeTab]);
  ...
  useEffect(() => {
    ...
    if (musicEnabled === "true") setIsMusicPlaying(true);
      ...
      <style jsx global>{`
        * { box-sizing: border-box; }
        ...
        @media (max-width: 375px) {
          nav button { padding: 8px 16px !important; font-size: 12px !important; letter-spacing: 0.3px !important; }
        }
      `}</style>

      <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0f172a 0%, #1e1b4b 25%, #312e81 50%, #1e1b4b 75%, #0f172a 100%)", backgroundSize: "400% 400%", color: "white", paddingBottom: "0" }}>
        <div style={{ padding: "clamp(20px, 5vw, 40px) clamp(16px, 4vw, 24px)" }}>
          ...
          <header style={{ maxWidth: "1400px", margin: "0 auto clamp(24px, 5vw, 40px)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px", flexWrap: "wrap" }}>
            ...
            <nav style={{ display: "flex", gap: "8px", padding: "4px", borderRadius: "12px", background: "rgba(15,23,42,0.8)", border: "2px solid rgba(139,92,246,0.3)" }}>
              {(['weekly', 'monthly'] as const).map((tab) => (
                <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: "10px 20px", borderRadius: "10px", border: "none", background: activeTab === tab ? "linear-gradient(135deg, #7c3aed, #d946ef)" : "transparent", color: activeTab === tab ? "white" : "#94a3b8", fontSize: "13px", fontWeight: 800, textTransform: "uppercase", cursor: "pointer", transition: "all 0.3s", letterSpacing: "0.5px" }}
          ...
          <main style={{ maxWidth: "1400px", margin: "0 auto" }}>
            ...
            <div className="animate-slide-up" style={{ padding: "clamp(32px, 6vw, 48px) clamp(24px, 5vw, 40px)", borderRadius: "clamp(20px, 4vw, 28px)", border: "2px solid rgba(251,191,36,0.5)", background: "linear-gradient(135deg, rgba(30,27,75,0.98) 0%, rgba(15,23,42,0.98) 100%)", boxShadow: "0 20px 60px rgba(0,0,0,0.6), 0 0 40px rgba(251,191,36,0.3)", backdropFilter: "blur(20px)", marginBottom: "clamp(24px, 5vw, 40px)", textAlign: "center" }}>
              ...
              {/* ═══ PRIZE BLOCK (DB karar verir) ═══ */}
              {data?.prize.type === 'money' ? (
                /* Monthly: £1000 + progress ring */
                ...
                /* Weekly: 15 credits (NO ring) */
                <div style={{ padding: "clamp(32px, 6vw, 48px) clamp(24px, 5vw, 40px)", borderRadius: "clamp(20px, 4vw, 28px)", background: "linear-gradient(135deg, rgba(139,92,246,0.25), rgba(124,58,237,0.2))", border: "3px solid rgba(139,92,246,0.6)", marginBottom: "clamp(24px, 5vw, 32px)", position: "relative", overflow: "hidden", textAlign: "center" }}>
            ...
            {loading ? (
              ...
              <div style={{ padding: "clamp(60px, 12vw, 100px)", textAlign: "center", borderRadius: "20px", border: "2px solid rgba(139,92,246,0.3)", background: "rgba(15,23,42,0.6)" }}>
              ...
              <>
                {/* ═══ TOP 3 PODIUM ═══ */}
                <div className="mobile-stack" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "clamp(16px, 3vw, 24px)", marginBottom: "clamp(32px, 6vw, 48px)", alignItems: "end" }}>
                ...
                {restPlayers.length > 0 && (
                  <div className="animate-slide-up" style={{ padding: "clamp(24px, 5vw, 32px)", borderRadius: "clamp(16px, 3vw, 24px)", border: "2px solid rgba(139,92,246,0.5)", background: "linear-gradient(135deg, rgba(30,27,75,0.98) 0%, rgba(15,23,42,0.98) 100%)", boxShadow: "0 20px 60px rgba(0,0,0,0.6)", backdropFilter: "blur(20px)", animationDelay: "0.3s" }}>
                    <h2 style={{ fontSize: "clamp(18px, 4vw, 24px)", fontWeight: 900, marginBottom: "clamp(20px, 4vw, 24px)", display: "flex", alignItems: "center", gap: "10px" }}>
                      <Sparkles style={{ width: "24px", height: "24px", color: "#a78bfa" }} />
                      Ranked Players
                    </h2>
                    <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                      {restPlayers.map((player, idx) => (
                        <div key={player.user_id} className="animate-slide-up" style={{ display: "flex", alignItems: "center", gap: "clamp(12px, 2.5vw, 16px)", padding: "clamp(12px, 2.5vw, 16px)", borderRadius: "14px", background: "rgba(139,92,246,0.05)", border: "1px solid rgba(139,92,246,0.2)", transition: "all 0.3s", cursor: "pointer", animationDelay: `${Math.min(idx * 0.03, 0.5)}s` }}
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
                              <span style={{ fontSize: "clamp(14px, 3vw, 16px)", fontWeight: 800, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{player.full_name}</span>
                          ...
                          <div style={{ padding: "clamp(8px, 2vw, 12px) clamp(12px, 2.5vw, 20px)", borderRadius: "10px", background: "rgba(139,92,246,0.15)", border: "1px solid rgba(139,92,246,0.3)", textAlign: "right" }}>
                            <div style={{ fontSize: "clamp(16px, 3.5vw, 22px)", fontWeight: 900, background: "linear-gradient(90deg, #a78bfa, #f0abfc)", backgroundClip: "text", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{(player.total_score ?? 0).toLocaleString()}</div>
