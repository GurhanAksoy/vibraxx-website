...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    fetch()
    const id = setInterval(fetch, 15000) // refresh every 15s
    ...
    <>
      ...
      {health.critical_count > 0 && (
        <div style={{
          ...
          <span style={{ fontWeight: 700 }}>
            {health.critical_count} unresolved critical event{health.critical_count > 1 ? 's' : ''}
          </span>
          <span style={{ opacity: 0.7 }}>
            · last event {fmtAgo(health.last_event_at)}
          </span>
      ...
      <div className="grid-4" style={{ marginBottom: 24 }}>
        ...
        <StatCard
          ...
          value={pool.lowest_count?.toLocaleString() ?? '—'}
          sub={pool.lowest_category ?? 'no data'}
          variant={
            pool.lowest_count != null && pool.lowest_count < 500
              ? 'warn'
              : 'default'
          }
        />
